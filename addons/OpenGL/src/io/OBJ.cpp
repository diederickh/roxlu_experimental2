...
namespace gl {
  ...
  bool OBJ::copyVertices(std::vector<VertexP>& verts) {
    ...
    for(std::vector<FACE>::iterator it = faces.begin(); it != faces.end(); ++it) {
      FACE& f = *it;
      verts.push_back(VertexP(vertices[f.a.v]));
      verts.push_back(VertexP(vertices[f.b.v]));
      verts.push_back(VertexP(vertices[f.c.v]));
    }
